    ...
    methods:
      {
       ...
       getMemberInfo() {
			...
			$http.get('member.member.getUserInfo').then(function (response) {
				that.pay_passwad=response.data.yz_member.yz_pay_password;
       	     console.log('支付密码:',response.data.yz_member.yz_pay_password)
       	   })
       },
        confirm(btn) {
          var that = this;
          ...
          if (type == 1) {
            ...
            //order.pay.alipay , order_id
             that.$router.push(that.fun.getUrl('alipay', { status: 2, recharge_money: this.money, order_pay_id: that.order_pay_id, uid: that.uid,pid:this.$route.query.pid?this.$route.query.pid:window.localStorage.getItem("uid") }));
          } else if (type == 9) {
            that.appWeiPay();
          } else if (type == 10) {
            that.appAliPay();
        ...
        cloudAlipay() {
          var that = this;
          let json = { order_pay_id: that.order_pay_id, pid:this.$route.query.pid?this.$route.query.pid:window.localStorage.getItem("uid")};
        ...
        balancePay(password) {
          var that = this;
          let json = { order_pay_id: that.order_pay_id, "i": that.fun.getKeyByI(), "type": that.fun.getTyep() ,pid:this.$route.query.pid?this.$route.query.pid:window.localStorage.getItem("uid")};

          if (!this.fun.isTextEmpty(password)) {
            json = { order_pay_id: that.order_pay_id, "i": that.fun.getKeyByI(), "type": that.fun.getTyep(), payment_password: password ,pid:this.$route.query.pid?this.$route.query.pid:window.localStorage.getItem("uid")};
          }
...
//            that.$router.go(-1);
           if(this.$route.query.num>0){
           	that.$router.push(that.fun.getUrl('grouplist',{},{ status:'0'}))
           }else{
			that.$router.push(that.fun.getUrl('orderlist',{ status:'1'}))
			 }
            } else {
              MessageBox.alert(response.msg, '提示');
            }
        ...
        appWeiPay() {
          var that = this;
          $http.get('order.merge-pay.wechatAppPay', { order_pay_id: that.order_pay_id, "i": that.fun.getKeyByI() ,pid:this.$route.query.pid?this.$route.query.pid:window.localStorage.getItem("uid")}).then(function (response) {
            if (response.result == 1) {
        ...
        cashPay() {
          ...
          this.$router.push(this.fun.getUrl('orderCash', { order_pay_id: this.order_pay_id ,pid:this.$route.query.pid?this.$route.query.pid:window.localStorage.getItem("uid")}));
        },

        appAliPay() {
          var that = this;
          $http.get('order.merge-pay.alipayAppRay', { order_pay_id: that.order_pay_id, "i": that.fun.getKeyByI() ,pid:this.$route.query.pid?this.$route.query.pid:window.localStorage.getItem("uid")}).then(function (response) {
            if (response.result == 1) {
                if (response.data.isnewalipay == 1) {
                    YDB.SetRSA2AlipayInfo(that.$store.state.temp.system.name, that.fun.getKeyByI(), that.money, that.order_sn);
                } else {
        ...
        getMultipleOrderInfo() {
          //order.pay.wechatPay
          var that = this;
          $http.get('order.merge-pay', { order_ids: this.order_ids ,pid:this.$route.query.pid?this.$route.query.pid:window.localStorage.getItem("uid")}).then(function (response) {
            if (response.result == 1) {
            	that.datas = response
        ...
        getWeChatPayParams() {
          //order.pay.wechatPay
          var that = this;
          $http.get('order.merge-pay.wechatPay', { order_pay_id: this.order_pay_id ,pid:this.$route.query.pid?this.$route.query.pid:window.localStorage.getItem("uid")}).then(function (response) {
            if (response.result == 1) {
...
//							}));
    ...
    activated() {
      ...
      this.groupnum=this.$route.query.num;
        console.log('判断拼团',this.groupnum);
