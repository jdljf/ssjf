<template>
	<div class="comment_allgoods"
		...
		<div class="comment_item"
		     v-for="(good,index) in has_many_order_goods">
			<div class="goods">
				<a 
			...
			<div class="foot">
				<div class="action">
					<span class="btn btn_1"
					      @click="goEvaluate($event,index)">{{!disabled[index]?'填写评价':'查看评价'}}<i class='fa' :class="{'fa-angle-down':index==display,'fa-angle-right':index!=display}"></i></span>
				</div>
			</div>
			...
			<div class="inner"
				...
				<div class="comment_view">
					<template v-if='iscomment'>
						...
						<div class="stars"
							...
							<el-rate v-if="!comments[index]"  v-model="level"
							         show-text
							         @change="getStar"></el-rate>
							<el-rate v-if="comments[index]"  v-model="level" disabled show-text></el-rate>        
						</div>
					</template>
					<template v-if='!iscomment'>
						<span class="label"><el-rate v-model="comments[index]" disabled text-color="#ff9900" text-template=""></el-rate></span>
					</template>
				</div>
				<div class="comment_textarea">
					<div class="textarea_wrap">
						<template v-if='!disabled[index]'>
							<el-input :disabled="disabled[index]"
							          type="textarea"
							          :rows="2"
						...
						<template v-if='disabled[index]'>
							<div class="option">{{comments[index]}}</div>
						</template>
					</div>
				</div>
				...
				<template v-if='!disabled[index]'>
					<div class="mod_btns">
						<button :class="btn_class[index]"
						        @click="toComment(good,index)">发表评价</button>
					</div>
				</template>
			</div>
