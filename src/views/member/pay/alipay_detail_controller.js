...
export default {
    ...
    methods: {
        ...
        toPayOrder() {
            ...
            var that = this;
            $http.get('order.merge-pay.alipay', { pid:this.$route.query.pid?this.$route.query.pid:window.localStorage.getItem("uid"),uid: this.$route.params.uid, order_pay_id: this.$route.params.order_pay_id, "i": this.fun.getKeyByI(), "type": this.fun.getTyep() }, '正在...').then(function (response) {
                if (response.result == 1) {
                    that.alipay_url = response.data;
                    that.webToPay();
                } else {
