...
export default {
	data() {
		return {
			cartnum:0,
			has_one_endorse:{},//专家背书
			endorshow:true,//是否显示专家背书
			statuscode:false,
			goodsmsg:'',
			codestatus:'',
			group_order_sn:'',
...
//	}
	...
	activated() {
		this.hasgroup();
		this.group_order_sn=this.$route.query.group_order_sn;
		console.log('我传过来的group_order_sn',this.group_order_sn);
		if(this.$refs.hasimg){
			console.log('存在图片')
	...
	methods: {
		...
		initCservice(newCservice) {
			if(!this.fun.isTextEmpty(newCservice)) {
				...
				console.log('暂无消息',this.cservice);
				return;
  ...
  getData() {
   ...
   $http.get('goods.goods.get-goods', json, "加载中...").then(function(response) {
    ...
    that.has_one_goods_group_buying=response.data.has_one_goods_group_buying;
    if(response.data.has_one_goods_group_buying!=null){
    	that.has_one_goods_group_buying=response.data.has_one_goods_group_buying;
    that.Spellprice=response.data.has_one_goods_group_buying.price;
    that.Spellstatus=response.data.has_one_goods_group_buying.status;
     that.down_time=response.data.has_one_goods_group_buying.down_time;
     that.Spellnumber=response.data.has_one_goods_group_buying.number;
   
    	if(response.data.has_one_goods_group_buying.status==1){
    			that.footone=true;
    console.log('我是拼团',that.has_one_goods_group_buying);
    	}
    }else{
    	that.footone=false;
    console.log('我不是拼团');
...
//    console.log('专家背书',that.has_one_endorse);
    if(response.result == 1) {
   
    	if(response.data.has_one_endorse!=null){
    	that.has_one_endorse=response.data.has_one_endorse;
    var enstatus=that.has_one_endorse.status; 
     if(enstatus==1){
     	that.endorshow=true;
     }else{
     	that.endorshow=false;
     }     	
    }
    else{
     	that.endorshow=false;
		...
		getcarNum() {
			...
			$http.get('member.member-cart.index', json).then(function (response) {
				if (response.result == 1) {
					...
					for(var i=0;i<that.cartN.length;i++){
					cartlength+=parseInt(that.cartN[i].total);
					that.cartnum=cartlength;
					}
					console.log('购物车数量是',that.cartnum)
					that.cartN_l = that.cartN.length;
					console.log('关于购物车数据',that.cartN)
//					that.allCheckListInit(that.goods);
	...
	//初始化弹窗view
		initPopView() {
			...
			if(this.has_one_goods_group_buying!=null){
				this.popPrice=this.Spellprice;
				console.log('我是评团价格',this.popPrice)
			}
			else if(this.goodsInfo.has_option == 1) {
		...
		initPopView2() {
		    ...
		    else if(this.goodsInfo.has_option != 1&&this.goodsInfo.member_discount==false) {
				this.popPrice = this.goodsInfo.price; //设置默认价格
			}
		   else if(this.goodsInfo.member_discount!=false){
				this.popPrice = this.goodsInfo.member_discount.discount_value; //设置默认价格	
...
//				
		...
		buyNow() {
			...
			console.log('我的数据',submitActionTag);
			console.log('我的规格',optionsId);
			//判断是否有规格
...
//				this.buyNowRequest(goods_id, optionsId, this.goodsCount);
		     console.log('我传过来的group_order_sn21312',this.group_order_sn);
			this.buyNowRequest(goods_id, optionsId, this.goodsCount,this.Spellstatus,this.Spellprice,this.Spellnumber,this.down_time,this.group_order_sn);
				...
				//处理参数
//			this.buyNowRequest(goods_id, optionsId, this.goodsCount,this.Spellstatus,this.Spellprice,this.Spellnumber,this.down_time);
...
//				this.buyNowRequest(goods_id, optionsId, this.goodsCount);
		...
		//立即购买网络操作
		buyNowRequest(_goodsId, _optionsId, _total,_Spellstatus,_Spellprice,_Spellnumber,_down_time,group_order_sn) {
			if(optionsMaxCount == 0) //库存不足
			{
				Toast("商品库存不足");
				return;
						...
						MessageBox({
							...
							if(action == 'confirm') {
								that.$router.push(that.fun.getUrl('myinfo', {
									...
									optionsId: _optionsId,
									total: _total,
									Spellstatus:that.Spellstatus,
									Spellprice:that.Spellprice,
									Spellnumber:that.Spellnumber,
									down_time:that.down_time,
...
//                     console.log(this.goodsInfo.has_many_options[i].level_price_fronted);
		...
		hasgroup(){
            if(this.group_order_sn==undefined){
				this.group_order_sn='';
			}else{
		     this.group_order_sn=this.$route.query.group_order_sn;
			}
			$http.get("goods.goods.get-group-buying-status&id="+goods_id+"&group_order_sn="+this.group_order_sn).then(response => {
				if(response.result == 1) {
					var goodsarr= response.data;
					this.codestatus= response.data.code;
					this.goodsmsg=response.data.msg;
					if(this.codestatus==1||this.codestatus==2||this.codestatus==3){
					console.log('状态值是',this.codestatus);
					console.log('商品数量是',goodsarr);
		...
		//新处理立即购买
		buyNowNew() {
			console.log('我是评团参数状态',this.Spellstatus);
			console.log('我是评团参数价格',this.Spellprice);
			console.log('我是评团参数人数',this.Spellnumber);
			console.log('我是评团参数时间',this.down_time);
			console.log('我是评团参数22',this.group_order_sn);
			if(this.group_order_sn==undefined||this.codestatus==1||this.codestatus==2||this.codestatus==3){
				this.group_order_sn='';
			}else{
		     this.group_order_sn=this.$route.query.group_order_sn;
			}
		...
		postShow() {
			...
			$http.get("goods.goods-poster.generate-goods-poster", jsons, "...加载中").then(response => {
				if(response.result == 1) {
