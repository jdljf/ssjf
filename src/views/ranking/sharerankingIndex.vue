<template>
	<div id="sharerankdeatil">
		<c-title class="aa" style="background:red;" :hide="false" text='会员龙虎榜'>
			...
			<div class="texts">
				...
				<ul>
					...
					<li class="lis2" v-for="item in dataAll">
						...
						<span class="moneys">{{item.total}}</span>
					</li>
				</ul>
			</div>
...
<script>
	...
	export default {
		...
		methods: {
			getContentlist() {
				var that = this;
				$http.get("member.member.getLongHuBang").then((response) => {
					if(response.result == 1) {
						that.contentlist = response.data;
						that.dataAll = response.data.dataAll;
						console.log('数据是',that.contentlist);
